...
namespace UKPO2
{
    public class DNAGraph
    {
        ...
        public class Verticle
        {
            ...
            public ref List<Verticle> GetNeighbours() //Получение ссылки на список
            {
                return ref this.neighbours;
            }

            public void AddNeighbour(ref List<Verticle> list, int index)//Запись в список ссылки на значение из списка list
            {
                ...
                for (int j = 0; j < verticleList.Count; ++j)
                {
                    if (i != j)
                    {
                        if (verticleList[j].Index > verticleList[i].Index)
                        {
                            ...
                            {
                                verticleList[i].AddNeighbour(ref verticleList, j);
                            }
                        }
                    }
                }
